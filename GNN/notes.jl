...
# ╔═╡ bcab49d9-0c72-4028-81a9-c9c55fcc5558
using PlutoUI

# ╔═╡ d8c8bf90-8ef0-46f0-a8aa-ac104fb934ae
using Flux

# ╔═╡ 7d400939-5110-4128-b23a-608eaa6d0eb7
TableOfContents(title = "目录")

# ╔═╡ 63d1e0d1-97a4-422c-8025-722255839b34
md"""
这里主要是学习过程中一些重要函数的笔记
"""

# ╔═╡ c04b49a9-7b6f-4194-a684-f323149f151c
md"""
## Scatter 与 Gather
NNlib.gather 与 NNlib.scatter是非常基础的一个操作。 

NNlib.gather(src, idx) -> dst， NNlib.gather!(dst, src, idx)
实现的操作是： `dst[:, ... , k] .= src[:, ... , idx[k]...]`；

如果dst是矩阵， idx是向量， 则是
dst[:, k] .= src[:, idx[k]]

因此， gather实现的操作可以看成是从src这个数据源，去提取idx对应的那些元素。 这意味着， idx有多少个坐标， 提取的元素个数就有多少个。 idx的长度跟目标dst具有相同的长度。 
"""

# ╔═╡ 0e8cb293-ce27-4f29-84d5-44458be9ac71
NNlib.gather([1 2 3; 4 5 6], [1,3,1,3,1])

# ╔═╡ 20168fdc-aa1a-4019-86df-3c306e4f08b5
md"""
NNlib.scatter(op, src, idx; [init, dstsize]) -> dst

NNlib.scatter!(op, dst, src, idx)

实现的是`dst[:, ..., idx[k]...] = (op).(dst[:, ..., idx[k]...], src[:, ..., k...])`

相当于将源数据src中坐标为1：n的元素  赋值给目标dst的对应坐标为idx 元素。 或者， 依次将src中的元素放入目标的idx位置上， 不过在放上去的时候，需要跟目标位置上的元素做一个op操作。

所以， 在sactter时， src的长度跟idx的长度相同。 **依次分派运算scatter， 可重复搜集gather。**
"""

# ╔═╡ e1b22c64-3644-43dc-85f5-71ced15b2a3b
NNlib.scatter!(+, ones(3), [10,100], [1,3])

# ╔═╡ c8562e65-a042-47b2-bc4f-a109c200f3c1
md"""
## apply_edges函数
Apply\_edges函数将节点上的特征序列化到边上， 并应用消息函数。 其源代码如下：
```julia
...
```
每一条边， 它必然有一个源节点和目标节点。我们假定给所有节点一个编号，从1到nv(g)。这样的话， 一条边可以用f->t表示。 

s, t = edge_index(g)获取图的坐标表示。 s,t表示了图中每一条边的源（source）节点坐标和目标（target）节点坐标； 节点是按照`1:nv(g)`自然编号的。

GNNGraphs.\_gather(xi, t) 实现从xi中把目标节点（t）的特征拿过来组成一个矩阵。 所以， 这里**xi**可以看成是每一个节点作为**目标节点（t）**时的消息。 而GNNGraphs.\_gather(xj, s)实现从**xj**中给每一个源节点提取信息。**xj**可以看成是每一个节点， 作为**源节点(s)**时它的消息。 消息传递的方向是**j -> i**。 e本身就是边上的信息。 

这样， 经过 `m = f(xi, xj, e)` 意味着对每一条边通过 加工输入节点的信息、输出节点信息和节点本身的信息得到最终的边上的消息。

"""

# ╔═╡ ce271afa-cdc7-4cf3-a01e-e43f13b5f6be
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "1134b9057009e3b942ba6c2609b60279c2b9b589"
