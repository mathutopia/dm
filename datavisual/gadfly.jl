...
```
其中， 第一个参数data是要绘制的**数据**（对应数据层,通常是dataframe或数组）， 最后的关键字参数将各种**图形属性**映射到数据框架中的**列**（变换层）。数据和映射之间的所有输入参数是图形的“**几何元素**”（比如，点、线、条形等）或者对图形要做的操作，通常对应各种图形。比如， 散点图是由点Geom.point构成。 这就相当于实现了一种图形语言。因此， 下面的每一个绘图操作， 都可以尝试用语言来描述。
...
# ╔═╡ 7748fb18-95d3-4bf3-a96a-9ecf9f7d7d28
plot(edudata, Geom.bar, x = :insured_education_level, y=:count , color = 1:7)
...
md"""
!!! warn "知识点"
	上面两个地方指定颜色的方式是通过给color这个绘图属性赋予相应的值， 可以是某个字段， 也可以是一些整数。 那么它是怎么转换为颜色的呢？ 事实上， 每一个图形属性， 都有一个对应的**标度变换Scale**， 这个变换会将数据集中的对应的数据转换为相应图形属性的数据。 比如，对颜色而言， 由于这里是离散型变量， 会有一个Scale.discrete_color,将离散值（这里是7种学历水平，或1:7）， 转换为一个调色板上的7个颜色。通常我们会有一个默认的调色板。 不过你可以修改为你喜欢的调色板。 
